...
int db_add(DB* self, Variant* key, Variant* value)//epistrefei 1
{
    pthread_mutex_lock(&self->writer);//eisodos se krisimh perioxh amoibaios apokleismos me locks 

     //vazw kleidi kai value sto memtable h sto sst ama xreiazetai compaction
    int memtableadd=0;	
    ...
    pthread_mutex_unlock(&self->writer);//eksodos apo krisimh perioxh
    //apothikeyw thn epistrofh ths sunarthshs sthn metvlhth memtableadd epeidh to nhma ua prepei na dei thn synarthsh epeidh epistrefei synarthsh ara prin to unlock kai thn apothikeyv se metablhth wste to epomeno nhmana kalesei neasynarthsh
    return memtableadd;
}
...
int db_get(DB* self, Variant* key, Variant* value)
{
    ...
    pthread_mutex_lock(&self->reader);//amoivaios apokleismos
    self->readers_num=addreadnum(self->readers_num);//prosthetw wste na exw to poly enan reader wste na mhn afhsw kapoion writer na treksei mesa ston reader	
    if (self->readers_num==1)//lock ama vrei kapoion writer mesa sto read 
		pthread_mutex_lock(&self->writer);
    pthread_mutex_unlock(&self->reader);
	
    int sstget=0;
    //paei prwta elegxei sto memtable an den brethei
    if (memtable_get(self->memtable->list, key, value) == 1)
        sstget=1;
    //kanei anazhthsh sto sst kai to epistrefei	 	
    sstget=sst_get(self->sst, key, value);
    

    pthread_mutex_lock(&self->reader);//eisodos se krisimh perioxh
    self->readers_num =subreadnum(self->readers_num);//afairw enan reader mexris otoy ginei 0 kai afhnw touw writes na treksoun
    if (self->readers_num==0)//vgainei apo readers kai afhnei writers na treksoun
		pthread_mutex_unlock(&self->writer);
    pthread_mutex_unlock(&self->reader);//eksodos apo krisimh perioxh	
